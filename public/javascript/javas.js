...
$(document).ready(function () {
    ...
    var row=$('#ratesCoins tr').eq(1);
    var ths=$('#ratesCoins').find('tr').find('th');
    var lungime=ths.length;
    var valoarecurenta=0;
    var date = {};
    date.value = 1;
    date.currency1= $('#ratesCoins').find('tr').eq(1).find('td').eq(0).text();
    for(var i=1; i< lungime-1; i++){
        (function(i){
            date.currency2 = ths.eq(i).text();
            console.log(ths.eq(i).text());
            $.ajax({
                url: "../../api/convert",
                type: "POST",
                data: date,
    ...
    var ths= $('#ratesApi tr th');
    var lung=ths.length;
    for(var i=1; i<lung-1; i++){
        (function(i){
            var real="";
            $.ajax({
                url: "../api/coins/digital/" + $('#coin').text() + "/apiTickers/"+ths.eq(i).text(),
                type: "GET",
                dataType: "json",
                success: function (data) {
                    real=data.realsname;
